    ...
    return (
      <div className="empty-state">
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginTop: 16 }}>
          ...
          <button className="btn btn-p" onClick={() => nav('')}>返回市场</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="detail-wrap">
      ...
      <div className="detail-info">
        <h2>{tpl.name} {tpl.ai && <span className="chip-btn" style={{ background: 'var(--primary-grad)', color: '#fff', border: 'none', cursor: 'default', verticalAlign: 'middle' }}>AI 生成</span>}</h2>
        <div className="tags"><span>{tpl.category}</span><span>{tpl.style}</span>{tpl.ai && <span style={{ background: 'rgba(244,114,182,.12)', color: '#DB2777' }}>可编辑 · 可下载</span>}</div>
        <p className="desc">{tpl.description}</p>
        <div className="acts">
          <button className="btn btn-p" onClick={useIt}>✏️ 使用此模板（编辑）</button>
          <button className="btn btn-line" onClick={() => downloadTemplate(tpl, structuredClone(tpl.defaults), `${tpl.id}.html`)}>⬇ 直接下载</button>
          <button className="btn btn-line" onClick={() => nav('')}>← 返回市场</button>
          {tpl.ai && <button className="btn btn-danger" onClick={remove}>🗑 删除此模板</button>}
        </div>
