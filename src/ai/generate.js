...
3. 排版细节：建立清晰字体层级（标题/副标题/正文/标注 4 级，字号字重逐级递减）；标题用 display 字体（衬线如 Playfair/思源宋体，或几何无衬线如 Poppins/Space Grotesk）；正文 16px 行高 1.6-1.75、行长 60-75 字符；数字/代码用等宽字体。**西文与代码字体直接使用 Inter / Space Grotesk / Poppins / JetBrains Mono（页面已自动加载，无需自建 @font-face）；中文标题可用系统衬线栈（Georgia,'Songti SC','Noto Serif SC','SimSun',serif）或系统黑体栈；禁止在 CSS 里写未加载的自定义 font-family。** 注意中文排版纪律：中文大标题上限约 80px、行高 1.15-1.25、不要负字距堆叠；正文 max-width 约 28em；不要给中文用 text-transform:uppercase。
