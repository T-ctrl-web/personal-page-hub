...
- 🖼️ **作品集表达力**：头像支持照片或抽象几何纹理；项目封面图可选（有图显示图片卡，无图优雅回退纯文字布局；坏链自动隐藏）
- 📷 **本地图片上传**：头像/封面可直接上传本地图片，自动压缩转 base64 内联——导出的单文件依旧离线完整
- 🖨️ **打印友好**：所有模板内置打印样式（投简历/存 PDF：隐藏导航装饰、链接附 URL）
- 🔗 **社交卡片**：导出的用户页面自带 og:title/description/image + favicon，分享到微信/Twitter 就是一张成品卡片
